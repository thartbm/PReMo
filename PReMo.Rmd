...
PReMo_model <- function(parfree, parset, schedule) {
  ...
  for (trial in c(1:length(schedule))) {
    ...
    if (is.na(schedule[trial])) {
      x_vt <- Gt + 0 # clamps are NA, and in our data always zero-clamped... need extra functionality
    } else {
...
PReMo_fit <- function(schedule, 
  ...
  if (dim(searchgrid)[1] > 2000) {
    searchgrid <- searchgrid[sample(c(1:dim(searchgrid)[1]),size=2000),]
  }
  
  MSEs <- apply(searchgrid,FUN=PReMo_errors,MARGIN=c(1),parset=parset,schedule=schedule,reaches=reaches,proprioception=proprioception)
...
lines(act_adapt, col='red')
lines(act_prop, col='blue')
lines(behavior$reach, col='red')
lines(behavior$prop, col='blue')
```
